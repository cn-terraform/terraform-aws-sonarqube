...
resource "aws_iam_role" "ecs_task_execution_role" {
    name               = "${var.name_preffix}-sonar-ecs-task-execution-role"
...
resource "aws_iam_role" "ecs_autoscale_role" {
    name               = "${var.name_preffix}-sonar-ecs-autoscale-role"
    assume_role_policy = "${file("${path.module}/files/iam/ecs_autoscale_iam_role.json")}"
}
resource "aws_iam_role_policy" "ecs_autoscale_role_policy" {
    name   = "${var.name_preffix}-sonar-ecs-autoscale-role-policy"
